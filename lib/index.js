...
module.exports = function(options) {
  ...
  var middleware = function(req, res, next) {
    ...
    if (req.method === 'OPTIONS' && req.headers.has('access-control-request-headers')) {
      var hasAuthInAccessControl = !!~req.headers.get('access-control-request-headers')
    ...
    } else if (req.headers && req.headers.has('authorization')) {
      var parts = req.headers.get('authorization').split(' ');
      if (parts.length == 2) {
        var scheme = parts[0];
  ...
  middleware.unless = unless;//TODO: unless needs a port too
